...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://blogz:achieve@localhost:8889/blogz'
app.config['SQLALCHEMY_ECHO'] = True
db = SQLAlchemy(app)
app.secret_key = 'as12df34'
...
def new_post():
    if request.method == 'POST':
        ...
        if blog:
            ...
            return redirect("/newpost?tit_error=" + error)
        if blog_title =='':
            error = "Please specify the blog title."
            return redirect("/newpost?tit_error=" + error)
        if blog_body =='':
            error = "Please specify the blog content."
            return redirect("/newpost?bod_error=" + error+"&title=" +blog_title)
    ...
    tit_error = request.args.get("tit_error")
    bod_error = request.args.get("bod_error")   
    blog_title = request.args.get("title")
    
    if not blog_title:
        blog_title='' 
    return render_template('newpost.html',pgtitle='Add A Blog Post',tit_error=tit_error,bod_error=bod_error,title = blog_title)
